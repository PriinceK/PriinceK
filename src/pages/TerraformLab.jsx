...
function highlightTerraform(code) {
  ...
  return lines.map((line, i) => {
    ...
    let remaining = line
    let pos = 0
    const TF_KEYWORDS = /\b(resource|variable|output|provider|data|module|locals|terraform|for_each|count|depends_on|lifecycle|dynamic|provisioner)\b/g
    const TF_TYPES = /\b(string|number|bool|list|map|set|object|tuple|any|null|true|false)\b/g

    // Simple regex-based highlighting
    const highlighted = remaining
      .replace(/("(?:[^"\\]|\\.)*")/g, '‹STR›$1‹/STR›')
      .replace(TF_KEYWORDS, '‹KW›$&‹/KW›')
      .replace(TF_TYPES, '‹TY›$&‹/TY›')
      .replace(/(\{|\}|\[|\])/g, '‹BR›$&‹/BR›')
      .replace(/(=)/g, '‹OP›$&‹/OP›')

    const parts = highlighted.split(/(‹\/?(?:STR|KW|TY|BR|OP)›)/g)
    ...
    parts.forEach((part, j) => {
      ...
      if (openMatch) { currentStyle = STYLE_MAP[openMatch[1]] || null; return }
      if (closeMatch) { currentStyle = null; return }
      if (part) {
        result.push(currentStyle ? <span key={j} style={currentStyle}>{part}</span> : part)
      }
    })
...
export default function TerraformLab() {
  ...
  const highlighted = useMemo(() => highlightTerraform(code), [code])
    ...
    <div className="max-w-6xl mx-auto px-4 py-10">
      ...
      <div className="grid lg:grid-cols-2 gap-4 mb-6">
        ...
        <div className="glass-card-static rounded-xl overflow-hidden">
          ...
          <div className="relative h-80 bg-[#0a0e1a]">
            <pre
              ref={preRef}
              className="absolute inset-0 p-4 font-mono text-xs leading-relaxed overflow-auto pointer-events-none m-0 whitespace-pre-wrap break-words"
              aria-hidden="true"
            >{highlighted}</pre>
            <textarea
              ref={textareaRef}
              ...
              className="absolute inset-0 w-full h-full p-4 bg-transparent text-transparent caret-nebula-text font-mono text-xs leading-relaxed resize-none border-0 outline-none"
              spellCheck={false}
